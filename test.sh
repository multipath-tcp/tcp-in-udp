...
setup()
{
	...
	ip -n "${NS}_cpe" route add default via 10.0.1.1 dev "net"
	tc -n "${NS}_cpe" qdisc add dev "net" root netem rate 10mbit delay 5ms
	...
	ip -n "${NS}_net" route add default via 10.0.1.2 dev "cpe"
	tc -n "${NS}_net" qdisc add dev "cpe" root netem rate 10mbit delay 5ms
